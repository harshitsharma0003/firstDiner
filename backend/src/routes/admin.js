...
// Normalise an Indian phone number to E.164 (+91XXXXXXXXXX) for WhatsApp/SMS.
function normalizeIndianPhone(raw) {
  ...
  return String(raw).trim().startsWith('+') ? String(raw).trim() : (digits ? `+${digits}` : '');
}
...
router.post('/restaurants', async (req, res) => {
  const { name, description, location, images, username, email, phone } = req.body || {};
  ...
  const ownerPhone = normalizeIndianPhone(phone); // '' if not given
  ...
  const restaurant = await store.createRestaurant({
    ...
    images: images || [],
    phone: ownerPhone, // restaurant contact for booking WhatsApp/SMS alerts
    maxTables: 5,
